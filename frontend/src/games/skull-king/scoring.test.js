...
describe('calculateRoundScore', () => {
  ...
  it('davy jones leviathans → +20 each (bid met)', () => {
    ...
    expect(r.bonusScore).toBe(40);
    expect(r.totalRoundScore).toBe(60);
  });
  ...
  it('8★ captured → +5 each (bid met)', () => {
    ...
    expect(r.totalRoundScore).toBe(55);
  });
...
describe('calculateAllRoundScores', () => {
  it('calculates for multiple players', () => {
    const scores = calculateAllRoundScores(
      [makeData({ playerId: 'p1', bid: 2, tricks: 2 }), makeData({ playerId: 'p2', bid: 0, tricks: 0 })],
      3,
    );
...
describe('computeCumulativeScores', () => {
  it('sums across rounds', () => {
    const totals = computeCumulativeScores([
      {
        roundNumber: 1,
        playerData: [makeData({ playerId: 'p1', bid: 1, tricks: 1 }), makeData({ playerId: 'p2', bid: 0, tricks: 0 })],
      },
      {
        roundNumber: 2,
        playerData: [makeData({ playerId: 'p1', bid: 0, tricks: 0 }), makeData({ playerId: 'p2', bid: 2, tricks: 2 })],
      },
    ]);
